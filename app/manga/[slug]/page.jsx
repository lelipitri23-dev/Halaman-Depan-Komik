...
export async function generateMetadata({ params }) {
  ...
  return {
    title: manga.title,
    description: `Baca ${manga.title} - ${manga.type || 'Komik'} ${manga.status || ''}. ${manga.synopsis?.slice(0, 120) || ''}`,
    openGraph: {
      title: manga.title,
      description: `Baca ${manga.title} di Komikcast gratis!`,
      images: [{ url: manga.coverImage || '', alt: manga.title }],
      type: 'book',
    },
...
export default async function MangaDetailPage({ params }) {
  ...
  const jsonLd = {
    ...
    author: { '@type': 'Organization', name: manga.author || 'Unknown' },
      ...
      <main className="pt-14 pb-safe max-w-2xl mx-auto">
              ...
              <div className="flex items-center gap-4 flex-wrap mb-2">
                {manga.rating > 0 && (
                  <div className="flex items-center gap-1">
                    ...
                    <span className="font-bold text-accent-gold text-sm">{manga.rating?.toFixed(1)}</span>
                  </div>
                )}
                <div className="flex items-center gap-1">
        ...
        {manga.synopsis && (
          <div className="px-4 mt-4">
            ...
            <p className="text-text-secondary text-sm leading-relaxed">{manga.synopsis}</p>
          </div>
        )}
          ...
          <div className="px-4 mt-6">
            ...
            <div className="grid grid-cols-3 gap-3">
              {recommendations.map((m) => (
                <MangaCard key={m._id || m.slug} manga={m} />
              ))}
            </div>
          </div>
        )}
