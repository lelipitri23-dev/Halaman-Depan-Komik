...
export async function generateMetadata({ params }) {
    ...
    const pageTitle = `Baca ${manga?.type || 'Komik'} ${manga.title} Bahasa Indonesia`;
    const pageDesc = `Baca ${manga?.type || 'Komik'} ${manga.title} Download ${manga?.type || 'Komik'} ${manga.title} Bahasa Indonesia.`;
    const pageUrl = `${SITE_CONFIG.baseUrl}/manga/${slug}`;
    
    // FIX: Menggunakan manga.nativeTitle dan manga.genres sesuai struktur database
    const keywords = [manga.title, manga.nativeTitle, manga?.author, manga?.artist, ...(manga.genres || [])].filter(Boolean).join(', ');
...
export default async function MangaDetail({ params }) {
    ...
    // 2. AMBIL DATA
    // FIX: Ambil array chapters DARI DALAM objek info (manga.chapters)
    ...
    // FIX: Format tanggal agar tampil rapi
    const formattedLastUpdated = manga?.lastUpdated 
        ? new Date(manga.lastUpdated).toLocaleDateString('id-ID', { year: 'numeric', month: 'long', day: 'numeric' })
        : '-';
                ...
                <div className="container mx-auto px-4 relative z-10 pt-8 sm:pt-12">
                    <div className="flex flex-col md:flex-row gap-8 items-start">
                        ...
                        <div className="w-[180px] sm:w-[240px] flex-shrink-0 mx-auto md:mx-0 rounded-lg overflow-hidden shadow-2xl border border-gray-700/50 aspect-[3/4] relative group">
                            {manga.coverImage ? (
                                <Image src={manga.coverImage} fill className="object-cover transition-transform duration-500 group-hover:scale-105" alt={manga.title} unoptimized />
                            ) : (
                        ...
                        <div className="flex-1 w-full">
                            ...
                            {/* FIX: Menggunakan manga.nativeTitle */}
                            {manga.nativeTitle && (
                            ...
                            <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 md:gap-6 text-sm text-gray-400 mb-6 border-y border-gray-800 py-4">
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <strong className="text-white">{manga?.rating || '0'}</strong>
                                </div>
                                <div className="flex items-center gap-1.5">
                                    <List size={18} />
                                ...
                                <div className="flex items-center gap-1.5">
                                    <Eye size={18} />
                                    {/* Asumsi: Kamu belum punya views di model baru, kasih nilai default dulu */}
                            ...
                            <div className="entry-content entry-content-single maincontent mb-8 text-sm text-gray-400 bg-gray-900/30 p-4 rounded-lg border border-gray-800/50" itemProp="description">
                                <div className="chdesc">
                                    <p>
                                        Baca {manga?.type || 'Komik'} <b className="text-white">{manga.title}</b> {latestChapter ? `Chapter ${latestChapter.title}` : ''} bahasa Indonesia terbaru di <b className="text-white">{SITE_CONFIG.name}</b>.
                                        {manga?.type || 'Komik'} <b className="text-white">{manga.title}</b> bahasa Indonesia selalu update di <b className="text-white">{SITE_CONFIG.name}</b>.
                                        Jangan lupa membaca update manga lainnya ya. Daftar koleksi {manga?.type || 'Komik'} ada di menu Daftar.
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="space-y-4 bg-black/20 p-4 rounded-xl border border-gray-800/50">
                                ...
                                <MetaRow label="Author">
                                    <Pill text={manga?.author || '-'} />
                                </MetaRow>

                                <MetaRow label="Artist">
                                    <Pill text={manga?.artist || manga?.author || '-'} />
                                </MetaRow>
                                <MetaRow label="Updated">
                                    {/* FIX: Format Tanggal */}
                                    <Pill text={formattedLastUpdated} />
                                </MetaRow>
